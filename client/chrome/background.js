...
chrome.runtime.onMessage.addListener(sendserver);
function sendserver(request,sender,sendResponse){
    ...
    */
    if(is_phishing_client(request.data)){
        chrome.tabs.sendMessage(tabid,{success:true,is_malicious:true})
    }
    ...
    response.done(data => {
        if (typeof data.success !== 'undefined'){
            if (data.success == true){
                chrome.tabs.sendMessage(tabid,{success:data.success,is_malicious:data.is_malicious})
            }else{
                chrome.tabs.sendMessage(tabid,{success:data.success})
            }
        }
    });
    response.fail(data => {
